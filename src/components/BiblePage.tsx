...
import { loadBibleFromLocal } from '../utils/bibleFallback';
  ...
  const loadVerses = async () => {
    try {
      ...
      if (safeData.length > 0) {
        ...
        const fallback = loadBibleFromLocal();
        setVerses(fallback);
        setFilteredVerses(fallback);
      }
    } catch (error) {
      console.error('Error loading bible verses from Supabase, using local seed instead:', error);
      const fallback = loadBibleFromLocal();
      setVerses(fallback);
      setFilteredVerses(fallback);
    } finally {
