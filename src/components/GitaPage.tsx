...
import { loadGitaFromLocal } from '../utils/gitaFallback';
...
export default function GitaPage() {
  ...
  const loadVerses = async () => {
    try {
      ...
      // If Supabase is reachable and returned rows, use it; otherwise fall back to bundled dataset.
      if (safeData.length > 0) {
        setVerses(safeData);
        setFilteredVerses(safeData);
      } else {
        const fallback = loadGitaFromLocal();
        setVerses(fallback);
        setFilteredVerses(fallback);
      }
    } catch (error) {
      console.error('Error loading verses from Supabase, using local dataset instead:', error);
      const fallback = loadGitaFromLocal();
      setVerses(fallback);
      setFilteredVerses(fallback);
    } finally {
        ...
        <div className="space-y-6">
          {filteredVerses.map((verse) => (
            ...
            >
              ...
              <p className="text-2xl font-serif mb-2 text-gray-900 dark:text-gray-100">
                {verse.sanskrit || ''}
              </p>
              ...
              {verse.commentary && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-200">
                  <strong className="text-gray-900 dark:text-gray-100">Commentary:</strong> {verse.commentary}
                </div>
              )}

              {teluguExplanations[String(verse.id)] && (
                <div className="mt-4 pt-4 border-t border-blue-200 dark:border-blue-900/50 text-gray-800 dark:text-gray-200">
                  <strong className="text-blue-800 dark:text-blue-300">Telugu Explanation:</strong> {teluguExplanations[String(verse.id)]}
                </div>
              )}
            </div>
          ))}
